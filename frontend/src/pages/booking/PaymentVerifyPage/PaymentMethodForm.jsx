...
export default function PaymentMethodForm({
  ...
  const methodLabel = PAY_METHODS.find((item) => item.id === method)?.label
      ...
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-6 pb-28 sm:px-6 lg:flex-row lg:pb-8 lg:px-8">
        <div className="flex min-w-0 flex-1 flex-col gap-5">
          ...
          <section className="rounded-3xl border-2 border-line bg-white p-4 shadow-sm sm:p-5">
            ...
            <h2 className="mt-1 font-heading text-xl font-bold sm:text-2xl">Choose how to pay</h2>
            <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
              {PAY_METHODS.map((item) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`text-sm font-extrabold ${active ? 'text-secondary' : 'text-charcoal'}`}
                    >
                      {item.label}
                    </span>
                    <span
                      className={`mt-1 text-xs font-semibold ${active ? 'text-white/65' : 'text-slate'}`}
                    >
                      {item.hint}
                    </span>
                  </button>
          ...
          <section className="rounded-3xl border-2 border-line bg-white p-4 shadow-sm sm:p-5">
            ...
            <div className="mt-3 flex flex-col">
              ...
              <DetailRow label="Hold ID" value={payment.holdId} mono />
            </div>
          </section>
        </div>
        ...
        <aside className="hidden w-full flex-col lg:flex lg:w-[320px] lg:shrink-0 lg:self-start">
          <div className="overflow-hidden rounded-3xl border-2 border-line bg-white shadow-card">
            <div className="bg-charcoal p-5 text-white">
              ...
              <p className="mt-2 text-xs text-white/65">Via {methodLabel}</p>
            </div>
            <div className="flex flex-col gap-3 p-5">
              <Button
                className="w-full py-3.5 text-base disabled:cursor-not-allowed disabled:opacity-50"
                disabled={!hmacReady || verifying}
                onClick={onPay}
              >
                {verifying ? 'Verifying…' : `Pay ${formatMoney(amount)}`}
              </Button>
