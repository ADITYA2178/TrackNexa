...
export default function DatePickerField({ label, icon, value, onChange, className = '' }) {
  ...
  return (
    <div className={`relative rounded-xl border border-[#E4D7AD] px-4 py-3 ${className}`}>
      ...
      {open ? (
        <div className="absolute left-0 top-[calc(100%+0.5rem)] z-30 rounded-2xl border border-[#E4D7AD] bg-white p-3 shadow-2xl">
          <DayPicker
            mode="single"
            selected={value}
            onSelect={(date) => {
